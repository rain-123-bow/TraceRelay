...
def build_parser() -> argparse.ArgumentParser:
    ...
    commands.add_parser("start", help="start the detached Supervisor and Service")
    ...
    commands.add_parser("stop", help="close the session and stop both processes")
...
def _start_runtime() -> int:
    probe = ControlClient(timeout=3.0)
    try:
        response = probe.request({"command": "status"})
    except OSError as error:
        ...
        if not _is_trace_relay_status(response):
            return _write_start_error(
    ...
    while time.monotonic() < deadline:
        try:
            response = ControlClient(timeout=0.5).request({"command": "status"})
        except (ControlProtocolError, OSError, TimeoutError) as error:
            last_error = error
            time.sleep(0.05)
            continue
        if not _is_trace_relay_status(response):
            return _write_start_error(
                "control port was claimed by a non-TraceRelay protocol during startup"
            )
        response = dict(response)
        response.update(
            {
...
def _is_trace_relay_status(response: object) -> bool:
    ...
    return (
        ...
        and (
            (mode == "foreground" and supervisor_pid is None)
            or (
                mode == "managed"
