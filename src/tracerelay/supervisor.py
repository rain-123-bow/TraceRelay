...
def launch_detached_supervisor(timeout: float = 5.0) -> None:
    ...
    command = [sys.executable, "-m", "tracerelay.supervisor", "--detach"]
    try:
...
def run_supervisor(paths: RuntimePaths | None = None) -> int:
    ...
    service_process = context.Process(
        ...
        args=(service_connection, supervisor_pid, str(runtime_paths.root)),
...
def _detach() -> int:
    ...
    command = [sys.executable, "-m", "tracerelay.supervisor", "--run"]
    try:
...
def main() -> int:
    ...
    if sys.argv[1:] not in ([], ["--run"]):
        _write_stderr("unsupported Supervisor arguments")
        return 2
