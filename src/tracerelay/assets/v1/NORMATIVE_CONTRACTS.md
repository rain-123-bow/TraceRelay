...
overlapped operation uses an initially nonsignaled auto-reset event and retains
its `OVERLAPPED`, event, buffer, and file handle until one terminal completion
is observed. `CancelIoEx` requests cancellation but is not terminal proof; the
owner must obtain terminal completion with `GetOverlappedResultEx` or terminate
and observe the isolated owner process before releasing those objects. No
...
3. the coordinator creates the service and then the monitor as suspended
   ...
   anonymous-pipe handles, is resumed exactly once, and enters `PRE_READY`;
...
6. the coordinator and both children execute the exact command-EOF,
   ...
   eight bootstrap handle entries are proven closed, the coordinator
...
entries, 28 ready handle entries, 51 bounded pre-ready creation handle entries,
...
| 212 | 4 | maximum creation handle entries `51` |
...
SHA-256(
  ...
  le_u32(24) || le_u32(4) || le_u32(28) || le_u32(51) ||
...
child it creates two anonymous `CreatePipe` pairs: one coordinator-to-child
command pipe and one child-to-coordinator acknowledgement pipe. `CreatePipe`
creates the command pair first and acknowledgement pair second. It uses
`SECURITY_ATTRIBUTES { nLength=sizeof(SECURITY_ATTRIBUTES),
lpSecurityDescriptor=NULL, bInheritHandle=TRUE }` and zero requested
pipe-buffer bytes. Immediately after each pair's creation, the coordinator
clears `HANDLE_FLAG_INHERIT` on its retained endpoint. No pipe handle is
duplicated with `DuplicateHandle`.
...
|---:|---|---|---|---|---|
| 0 | coordinator | command write; `GENERIC_WRITE \| SYNCHRONIZE` (`0x40100000`); `WriteFile` only | no | no | after verified `FINAL_HANDLES_ACK`, before child observes command EOF |
| 1 | child | command read; `GENERIC_READ \| SYNCHRONIZE` (`0x80100000`); `ReadFile` only | yes, then cleared by child entry | yes | after command EOF, before `BOOTSTRAP_CLOSED_ACK` |
| 2 | child | acknowledgement write; `GENERIC_WRITE \| SYNCHRONIZE` (`0x40100000`); `WriteFile` only | yes, then cleared by child entry | yes | after `BOOTSTRAP_CLOSED_ACK` write returns |
| 3 | coordinator | acknowledgement read; `GENERIC_READ \| SYNCHRONIZE` (`0x80100000`); `ReadFile` only | no | no | after acknowledgement EOF |

The access contract is the documented directional access returned by
`CreatePipe`: every read end has `GENERIC_READ | SYNCHRONIZE` and every write
end has `GENERIC_WRITE | SYNCHRONIZE`. V1 does not substitute a queried
generic-to-specific `NtQueryObject.GrantedAccess` representation for this API
contract. A wrong-direction operation, duplicated endpoint, inheritance
outside the two declared child endpoints, or use after its close point is
invalid.
...
first pipe read. A failed membership proof, unexpected resume result,
primary-thread close failure, early child exit, pipe failure, or deadline
expiry is terminal startup failure; no resume retry is permitted.
...
positive returned byte counts until exactly 4,096 bytes. EOF before a complete
expected frame, zero progress, a read/write error, a frame outside this
sequence, nonzero reserved bytes, wrong role/phase/result/count, stale or
duplicate challenge, or any byte after the declared sequence is terminal
startup failure.
...
that all eight steady bootstrap handle entries are gone. None is serialized in
`TRIM/1`.
...
The exact all-process managed-handle inventory is:

| Checkpoint | Entries | Exact inventory |
|---|---:|---|
...
| monitor `CreateProcessW` returned, before duplicate child-end and primary-thread close | 14 | six surviving service-stage entries, four coordinator monitor-pipe endpoints, two inherited monitor pipe endpoints, one monitor-process handle, one primary-thread handle |
| both children resumed and creation duplicates closed | 11 | one coordinator job, two coordinator-held child-process handles, four coordinator pipe endpoints, four child pipe endpoints |
| all final mappings/events and handles allocated, bootstrap still open | 51 | 11 prior entries, 12 coordinator mapping/event handles, 24 duplicated child mapping/event handles, two child peer-process handles, two child startup-job handles |
| both bootstrap transports closed, before holder proof | 43 | 12 coordinator mapping/event handles, 24 child mapping/event handles, two coordinator child-process handles, two child peer-process handles, two child job handles, one coordinator job handle |
...
process, primary-thread, Job, mapping, event, peer-process and bootstrap-pipe
handle created, inherited, returned or duplicated for the IPC set across the
three processes. Journal, directory, installation-lock and other storage/control
handles are governed by their separate file, writer and control limits.
Current-process/thread pseudo-handles and interpreter/CRT internal handles are
also outside this domain.

Thus the child-process-creation transient maximum is exactly `14`, the
PRE_READY creation maximum is exactly `51`, and the ready maximum is exactly
...
byte, close coordinator pipe endpoints, terminate the startup Job, wait for
both held child process handles to become signalled, and close remaining
handles.
...
failure closes all handles it owns and exits. Each surviving child otherwise
waits only for the matching committed manifest inside its startup deadline;
missing, partial or invalid `TRIM/1` causes the same fail-closed exit. Partial
allocation/duplication, short/replayed/wrong-phase ACKs, and handle totals `52`
or greater never permit holder proof or `TRIM/1`. Boundary vectors must cover
`13/14/15`, `50/51/52`, each allocation/duplication, both child orders of
failure, and every holder-proof return.
...
| 348 | 4 | maximum creation handle entries `51` |
...
maximum pre-ready creation handle entries
  ...
  = 51
child-process-creation transient handle entries = 14
