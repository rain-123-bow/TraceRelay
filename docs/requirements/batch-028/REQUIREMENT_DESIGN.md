...
- Reviewer status: batch 027 passed its independent full scan with
  `P0=P1=P2=0`, closed all 113 historical findings, and was separately confirmed
  by the user. It remains the sole downstream requirement authority. Draft.28
  is the user-approved transport correction candidate; batch 028 review is
  pending.
...
| alarm IPC mappings / events / object handles / ready handles / creation handles | `4 / 8 / 24 / 28 / 54` |
| startup bootstrap | `TRBH/1`; two private local one-way byte-mode overlapped named-pipe pairs/child; 4,096-byte frames carrying the exact 3,328-byte pending `TRIM/1` inventory; 8 steady endpoints; at most 3 transient I/O events; 15 process-creation transient handles |
...
| ID | Requirement | Verification |
|---|---|---|
| TR-F-047 | Monitor shall execute in a process distinct from the service process and use a fresh monitor incarnation, installation lock, and monitor-owned alarm writer. The exact `TRBH/1` transport shall use two private local one-way byte-mode overlapped named-pipe pairs per child, serial pair construction, only the two directional client endpoints in each child `HANDLE_LIST`, and one non-inherited initially nonsignaled manual-reset event per active connect/read-all/write-all call retained through terminal completion. Service and monitor shall remain `PRE_READY` through the exact identity, final-handle and bootstrap-close sequence, and every surviving participant shall remain `PRE_READY` through matching committed `TRIM/1`. After exact child identity and Job-membership revalidation, the single kernel wait evaluation represented by `WaitForMultipleObjects(2, [service, monitor], FALSE, 0) == WAIT_TIMEOUT` shall freeze the acknowledged 28-handle holder-proof snapshot. Matching `TRIM/1` shall durably record that logical snapshot and shall not claim commit-time child liveness. Child loss before the holder-proof point shall block `TRIM/1`; child loss after it shall not cancel the one required manifest-commit attempt, shall remain a monitor/service failure, and shall prohibit external readiness. | Process-tree, identity, exclusive-lock, name/collision, server/client flag, direction, byte-mode, local-only, instance-count, connect immediate/pending/`ERROR_PIPE_CONNECTED`, manual-reset event ownership, cancel/terminal-reap, expected-versus-early `ERROR_BROKEN_PIPE`, byte-exact frame/kind/role/phase/count/padding/challenge/projection, ACK/EOF order, and every process/pipe/event/allocation/duplication crash boundary. Cover endpoint `7/8/9`, process-create `14/15/16`, structural `50/51/52`, actual PRE_READY `53/54/55`, post-close `42/43/44`, ready `27/28/29`, holder-proof return, replay, deadline, and no-early-readiness vectors. Cross both child roles with loss before/after the holder-proof point and every `TRIM/1` pre-witness byte boundary, flush-pending/returned boundary, witness prefix `0..44`, readback boundary, commit boundary and pre-readiness boundary. |
...
27. The `TRBH/1` startup transport retains its two one-way channels per child
    but uses private local byte-mode named pipes opened for overlapped I/O.
    Anonymous `CreatePipe` endpoints are prohibited because they cannot provide
    the required terminal-overlapped deadline and cancellation semantics.
...
- planned reviewer handle: `/root/tracerelay_requirement_change_reviewer_v2`
...
- batch 026 disposition: `TRR26-C-001` accepted into one draft.27 remediation;
  no per-finding edit/review cycle used
- batch 027 snapshot: `tracerelay-req-b027-48e1910c4369`
- batch 027 full scan: `PASS` (`P0=0`, `P1=0`, `P2=0`);
  `reviews/batch-027/REVIEW_RESULT.md`
- batch 027 historical closure: `113/113 CLOSED`; fresh findings `0`
- batch 027 user confirmation: recorded in `USER_CONFIRMATION.md`; batch 027
  remains the sole downstream authority
- batch 028 status: this draft.28 is the user-approved transport-change
  candidate; its identity will be supplied only by the enclosing immutable
